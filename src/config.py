...
_PG_URL = "postgresql+psycopg://{PG_USER}:{PG_PASSWORD}@{PG_HOST}:{PG_PORT}/{PG_DB}"
...
_REDIS_URL = "redis://{REDIS_HOST}:{REDIS_PORT}"
...
class DevelopmentSettings:
    ...
    SQLALCHEMY_DATABASE_URI = _PG_URL.format(
        PG_USER=PG_USER,
    ...
    CELERY = {
        "broker_url": _REDIS_URL.format(REDIS_HOST=REDIS_HOST, REDIS_PORT=REDIS_PORT),
        "result_backend": _REDIS_URL.format(REDIS_HOST=REDIS_HOST, REDIS_PORT=REDIS_PORT),
...
class TestSettings(DevelopmentSettings):
    ...
    LOGIN_DISABLED = True
    SQLALCHEMY_DATABASE_URI = _PG_URL.format(
        PG_USER=PG_USER_TEST,
        PG_PASSWORD=PG_PASSWORD_TEST,
        PG_HOST=PG_HOST_TEST,
        PG_PORT=PG_PORT_TEST,
        PG_DB=PG_DB_TEST,
    )
