...
def forgot_password() -> Any:
    ...
    if form.validate_on_submit():
        ...
        if user := get_user_by_uname_or_email_db(username_or_email=form.username_or_email.data):
            ...
            send_email(email_subject='(Flask Blog) Reset your password',
                       email_receivers=[user.email],
                       email_template='resetpswemail.html',
                       **{'user': user, 'url': url})
...
def reset_password(token: str) -> Any:
    ...
    if form.validate_on_submit():
        ...
        send_email(email_subject='(Flask Blog) Your password was reset',
                   email_receivers=[user.email],
                   email_template='infopswemail.html',
                   **{'user': user, 'url': url})
