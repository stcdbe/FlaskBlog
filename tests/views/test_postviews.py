...
from src.post.postservice import get_post_db, get_com_by_text_db
...
def test_create_post(client: FlaskClient,
    ...
    with app.app_context():
        post = get_post_db(title=post_data['title'])
        assert post
...
def test_show_post_detail(app: Flask,
    ...
    with app.app_context():
        post = get_post_db(title='test_title')
...
def test_update_post(client: FlaskClient,
    ...
    assert res_post.request.path == '/posts'
    with app.app_context():
        upd_post = get_post_db(title=upd_post_data['title'])
    assert upd_post
...
def test_delete_post(client: FlaskClient,
    ...
    assert res_post.request.path == '/posts'
    with app.app_context():
        deleted_post = get_post_db(title='test_title')
